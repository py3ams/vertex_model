...
global cell_figure_handle right_text_axes left_axes_1 left_axes_2 ...
    right_axes_1 right_axes_2 left_text_axes coloraxis
...
if ~new_visualiser_logical
    ...
    if ~iteration
...
else %if new_visualiser
	
		if ~iteration
			
			if view_FEM_mesh
			
				cell_figure_handle = figure('outerposition',[30 100 1300 650],'color','white');
			
				left_axes_1 = axes('position',[0 0 0.5 1]);
				figure_loop(cells,vertices,linewidth)
				axis(axis_values)
				axis off;
				
				right_axes_1 = axes('position',[0.5 0 0.5 1]);
				...
				if view_FEM_concentration
					...
					trisurf(FEM_element_nodes,FEM_nodes.previous_position(:,1),...
						FEM_nodes.previous_position(:,2),zeros(length(FEM_nodes.previous_position(:,1)),1),'linewidth',2)
					axis off;grid off;axis(axis_values_FEM);view(FEM_angle);shading(shading_style);
					hold on;
					for i =1:length(cells.vertices)
						hold on
						patchAS(vertices.position(cells.vertices{i},:),[50,200,50]/256,4)
					end
				...
				left_axes_1 = axes('position',[0 0 1 1]);
				figure_loop(cells,vertices,linewidth)
				axis(axis_values)
				axis off;
		...
		elseif movie_logical && ~rem(iteration,update_period_1) || final_iteration_logical
			...
			if view_FEM_mesh
				...
				if view_FEM_concentration
					...
					trisurf(FEM_element_nodes,FEM_nodes.previous_position(:,1),FEM_nodes.previous_position(:,2),...
						zeros(length(FEM_nodes.previous_position(:,1)),1),'linewidth',2);
					axis off;grid off;view(FEM_angle);axis(axis_values_FEM);
					hold on;
					for i =1:length(cells.vertices)
						hold on
