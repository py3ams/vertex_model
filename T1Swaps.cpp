...
//                printf("hello \n");
					...
					unsigned clockwise_vertex_global_mi = (unsigned)cell_vertices[clockwise_vertex_local];
					unsigned clockwise_vertex_global_ci = clockwise_vertex_global_mi - 1;
...
//                }
					...
					if(current_edge_length < threshold_T1_swaps &&
						...
						// only proceed if a cell_with_same_edge has been found. otherwise abort the whole thing. T1 swap cannot happen on
						// a boundary. at this point it is possible that current_vertex_global or clockwise_vertex_global might only belong to
						// two cells, whereas i think we need them to belong to exactly one other cell (i.e. the stretch cells) for it all to work?
						...
						if(cell_with_same_edge_found){
							...
							double* cell_vertices_cell_with_same_edge = mxGetPr(mx_cell_vertices_cell_with_same_edge);
							unsigned no_cell_vertices_cell_with_same_edge = mxGetN(mx_cell_vertices_cell_with_same_edge);
							
							// only proceed if cell with same edge has more than nodes
...
// 								printf("Doing a swap \n");
								...
								unsigned new_vertex_2_ci = new_vertex_1_ci+1;
								while(final_cells_per_vertex[new_vertex_2_ci]>0){
									new_vertex_2_ci++;
								}
								unsigned new_vertex_2_mi = new_vertex_2_ci+1;
...
// 								printf("found and stored new node positions \n");
								...
								// what happens if this is not true?? we won't find a stretch_cell_1, but this should be ok.
								// need to check that the stretch_cell exists before we try to do anything to its FEM elements/nodes
								// too
								if(final_cells_per_vertex[current_vertex_global_ci]==3){
								...
								if(final_cells_per_vertex[clockwise_vertex_global_ci] == 3){
