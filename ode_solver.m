function [vertices,angle_deviations,stats,vertex_movements] =...
...
if ode_solver_type == 1
   ...
   [vertices.position,angle_deviations,vertex_movements,current_total_area_force,...
...
elseif ode_solver_type == 2
   ...
   % Runge-Kutta is not set up to work with FEM solution yet as this
   % affects growth etc. we would need to pull all these parameters into
   % this function to make it work
   assert(length(FEM_nodes_concentration)==1)
   ...
   if cell_growth_logical
      
      RK_cells_for_b = cell_volume_growth(false,...
         RK_cells_for_b,delta_t/2,[],[],0,no_growth_time,[],time,vertices);
...
%       no_growth_time,time);
   ...
   if cell_growth_logical
      ...
      RK_cells_for_c = cell_volume_growth(false,...
         RK_cells_for_c,delta_t/2,[],[],0,no_growth_time,[],time,vertices);
...
%       no_growth_time,time);
   ...
   if cell_growth_logical
      ...
      RK_cells_for_d = cell_volume_growth(false,...
         RK_cells_for_d,delta_t,[],[],0,no_growth_time,[],time,vertices);
