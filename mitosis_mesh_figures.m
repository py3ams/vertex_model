...
if save_figs_logical
   ...
   export_fig(['Figs/pre_mitosis_cell.eps'],'-nocrop');
end
...
if save_figs_logical
   export_fig(['Figs/post_mitosis_cell.eps'],'-nocrop');
end
...
for i = 1:length(FEM_elements2)
    patchAS(FEM_vertex_positions(FEM_elements2(i,:),:),green1,linewidth_elements)
end
axis(axis_values)
axis off

if save_figs_logical
   export_fig(['Figs/temp_mitosis_cell.eps'],'-nocrop');
end
