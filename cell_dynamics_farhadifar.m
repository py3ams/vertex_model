...
rough_cell_area = 1/prod(grid_size);

% initial_force_constant_magnitudes.area = 1;
initial_force_constant_magnitudes.deformation = 0;
initial_force_constant_magnitudes.elongation = 0;
initial_force_constant_magnitudes.perimeter = -0.5*rough_cell_area;
% initial_force_constant_magnitudes.tension = 1*rough_cell_area^(3/2);
...
apoptosis_no_baseline_put_pc = 0.1;
...
while true
	...
	if update_positions_logical && iteration > update_positions_start
		...
		[vertices,stats,vertex_movements] =...
...
% 	cells.state(cells.state==2&cells.target_area-mean_cell_area>-1e-9) = 1;
    cells.state(cells.state==2&abs(time-cells.time_of_last_division)>0.5) = 1;
